...
#Region "変数定義"
    ...
    Dim dt生产 As New DataTable
#End Region

#Region "Form Event"

    Private Sub frm半成品入库申请_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub frm半成品入库申请_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Try
    ...
    Private Sub frm半成品入库申请_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        Dim frmSearch As frm生产实绩一览        Try
            ...
            If frmSearch.ReturnType = Constant.ENU_MSTSEARCH.SearchMOk Then
                ...
                If Not frmSearch.ReturnTable Is Nothing AndAlso 0 < frmSearch.ReturnTable.Rows.Count Then
                    ...
                    '根据检索画面数据进行设置                    If gridList.DataSource.select("订单编号='" & frmSearch.ReturnTable.Rows(0)("订单编号") & "'").length > 0 Then
                        If gridList.DataSource.select("分图号='" & frmSearch.ReturnTable.Rows(0)("分图号") & "'").length > 0 Then
                            CommonMsg.PFn_ShowMsg(Me.Text, Constant.ENU_MSGID.MSGID21, "分图号")
                        Else
                            gridList.ActiveRow.Cells("订单编号").Value = frmSearch.ReturnTable.Rows(0)("订单编号")          '原材料编号                            gridList.ActiveRow.Cells("主图号").Value = frmSearch.ReturnTable.Rows(0)("主图号")              '主图号                            gridList.ActiveRow.Cells("上级图号").Value = frmSearch.ReturnTable.Rows(0)("上级图号")
                            gridList.ActiveRow.Cells("分图号").Value = frmSearch.ReturnTable.Rows(0)("分图号")              '分图号                            gridList.ActiveRow.Cells("工序编号").Value = frmSearch.ReturnTable.Rows(0)("工序编号")
                            Setdava()
                        End If
                    Else
...
#Region "Button Event"
    ...
    Private Sub btnF4删除_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnF4删除.Click
        ...
        Try
            ...
            objZ半成品入库申请T.申请单编号 = txt申请单编号.Text.Trim
...
#Region "SetCombo"
    ...
    Private Sub SetCombo()
        ...
        Try
            Utility.PFn_SetGirdCombo(Me.gridList, "VM半成品种类", ENU_GRID.半成品种类)
            '交货方式
...
#Region "ClearForm"
    ...
    Private Sub ClearForm()
        ...
        Try
            ...
            Me.txt申请单编号.Text = "BCP" & Now.ToString("yyyyMMddhhmmss")
            Me.txt人员编号.Text = String.Empty
            Me.txt人员名称.Text = String.Empty
        ...
        Try
            ...
            If gridList.Rows.Count > 0 Then
                ...
                For i = 0 To gridList.Rows.Count - 1
                    objZ半成品入库申请T = New Z半成品入库申请T
                    objZ半成品入库申请T.申请单编号 = Me.txt申请单编号.Text.Trim
                    objZ半成品入库申请T.申请者编号 = Me.txt人员编号.Text.Trim
                    objZ半成品入库申请T.申请日期 = Me.dt申请日期.Text.Trim
                    objZ半成品入库申请T.半成品种类 = gridList.Rows(i).Cells("半成品种类").Value.ToString()
                    
                    objZ半成品入库申请T.订单编号 = gridList.Rows(i).Cells("订单编号").Value.ToString()
                    objZ半成品入库申请T.主图号 = gridList.Rows(i).Cells("主图号").Value.ToString()
                    objZ半成品入库申请T.上级图号 = gridList.Rows(i).Cells("上级图号").Value.ToString()
                    objZ半成品入库申请T.分图号 = gridList.Rows(i).Cells("分图号").Value.ToString()
                    'objZ半成品入库申请T.半成品名称 = gridList.Rows(i).Cells("半成品名称").Value.ToString()
                    objZ半成品入库申请T.工序编号 = gridList.Rows(i).Cells("工序编号").Value.ToString()
                    objZ半成品入库申请T.入库数量 = gridList.Rows(i).Cells("入库数量").Value.ToString()
                    objZ半成品入库申请T.备注 = gridList.Rows(i).Cells("备注").Value.ToString()


                    'objZ半成品入库申请T.客户名称 = gridList.Rows(i).Cells("客户名称").Value.ToString()
                    objZ半成品入库申请T.登陆者 = gObjM_人员T.人员编号
                    objZ半成品入库申请T.登陆日期 = DateTime.Now
                    objZ半成品入库申请T.更新者 = gObjM_人员T.人员编号
                    objZ半成品入库申请T.更新日期 = DateTime.Now
                    arryList.Add(objZ半成品入库申请T)
            ...
            Select Case mIntFormMode
                ...
                Case Constant.ENU_MODE.Model_New
                    ...
                    If objBll.LoadByPKey(objZ半成品入库申请T).申请单编号 <> String.Empty Then
                ...
                Case Constant.ENU_MODE.Model_Update
                    ...
                    If objBll.Delete(objZ半成品入库申请T) > 0 AndAlso objBll.Insert(arryList) > 0 Then
...
#Region "InputCheck"
    ...
    Private Function InputCheck() As Boolean
        ...
        Try
            ...
            Else
                For i = 0 To gridList.Rows.Count - 1
                    If gridList.Rows(i).Cells("半成品种类").Value.ToString() & "" = "" Then
                        CommonMsg.PFn_ShowMsg(Me.Text, Constant.ENU_MSGID.MSGID13, "半成品种类", "半成品种类")
                        Return False
                    End If
                    If gridList.Rows(i).Cells("分图号").Value.ToString() & "" = "" Then
                        CommonMsg.PFn_ShowMsg(Me.Text, Constant.ENU_MSGID.MSGID13, "分图号", "分图号")
                        Return False
                    End If
                    'If gridList.Rows(i).Cells("工序名称").Value.ToString() & "" = "" Then
                    '    CommonMsg.PFn_ShowMsg(Me.Text, Constant.ENU_MSGID.MSGID13, "工序名称", "工序名称")
                    '    Return False
                    'End If
                    'If gridList.Rows(i).Cells("半成品名称").Value.ToString() & "" = "" Then
                    ...
                    If gridList.Rows(i).Cells("订单编号").Value.ToString() & "" = "" Then
                        ...
                        Return False
                    End If
                    If gridList.Rows(i).Cells("主图号").Value.ToString() & "" = "" Then
                        CommonMsg.PFn_ShowMsg(Me.Text, Constant.ENU_MSGID.MSGID13, "主图号", "主图号")
                        Return False
                    End If
                    If gridList.Rows(i).Cells("入库数量").Value.ToString() & "" = "" Then
                    ...
                    'MsgBox("产品图号：" & objZ半成品入库申请T.分图号 & "不属于主图号：" & objZ半成品入库申请T.主图号)
                    ' Return False
                    '  End If
...
#Region "SetFormData"
    ...
    Private Sub SetFormData()
        Dim objZ半成品入库申请T As New Z半成品入库申请T
        Dim objBll As BLLZ半成品入库申请T

        Try
            '获取数据
            objZ半成品入库申请T = New Z半成品入库申请T
            objBll = New BLLZ半成品入库申请T

            objZ半成品入库申请T.申请单编号 = txt申请单编号.Text.Trim()

            If String.IsNullOrEmpty(objZ半成品入库申请T.申请单编号) Then
                objZ半成品入库申请T.申请单编号 = " "
            End If
            mdsInfoDataSet = objBll.LoadAllHead(objZ半成品入库申请T)
            ...
            mdsGridDataSet = objBll.LoadGridAll(objZ半成品入库申请T)
...
#Region "SetData"
    Private Sub SetData()
        ...
        Try
            ...
            objZ半成品入库申请T = New Z半成品入库申请T
            objBll = New BLLZ半成品入库申请T
            objZ半成品入库申请T.半成品名称 = gridList.ActiveRow.Cells("分图号").Value.ToString()

            If String.IsNullOrEmpty(objZ半成品入库申请T.分图号) Then
                objZ半成品入库申请T.分图号 = " "
            End If
            If gridList.Rows(0).Cells("分图号").Value.ToString <> String.Empty Then
                mDtSearch = objBll.SetData(objZ半成品入库申请T)
                If Not mDtSearch Is Nothing And mDtSearch.Rows.Count > 0 Then
                    ...
                    Me.gridList.ActiveRow.Cells("半成品名称").Value = mDtSearch.Rows(0)("品名")



                Else
                    Me.gridList.ActiveRow.Cells("半成品名称").Value = String.Empty
...
#Region "Setdava"
    Private Sub Setdava()
        ...
        Try
            ...
            'objZ半成品入库申请T.订单编号 = gridList.ActiveRow.Cells("订单编号").Value.ToString()
            objZ半成品入库申请T.主图号 = gridList.ActiveRow.Cells("主图号").Value.ToString()
            objZ半成品入库申请T.分图号 = gridList.ActiveRow.Cells("分图号").Value.ToString()
            objZ半成品入库申请T.上级图号 = gridList.ActiveRow.Cells("上级图号").Value.ToString()
            objZ半成品入库申请T.工序编号 = gridList.ActiveRow.Cells("工序编号").Value.ToString()


            dt生产 = objBll.Loaddava(objZ半成品入库申请T)
            If Not dt生产 Is Nothing And dt生产.Rows.Count > 0 Then
                Me.gridList.ActiveRow.Cells("半成品名称").Value = dt生产.Rows(0)("品名")
                Me.gridList.ActiveRow.Cells("工序名称").Value = dt生产.Rows(0)("工序名称")
            Else
                Me.gridList.ActiveRow.Cells("半成品名称").Value = String.Empty
                Me.gridList.ActiveRow.Cells("工序名称").Value = String.Empty
            End If
