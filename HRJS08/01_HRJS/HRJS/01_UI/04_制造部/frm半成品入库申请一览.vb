...
#Region "Form Event"
    ...
    Private Sub frm半成品入库申请一览_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub frm半成品入库申请一览_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Try
    ...
    Private Sub frm半成品入库申请一览_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
...
#Region "Grid Event"
    ...
    Private Sub gridlist_InitializeLayout(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.InitializeLayoutEventArgs) Handles grdList.InitializeLayout
    ...
    Private Sub grdList_ClickCel(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinGrid.ClickCellEventArgs) Handles grdList.ClickCell
        If e.Cell.Equals(grdList.ActiveRow.Cells(0)) Then
            Me.grdList.UpdateData()
        End If
    End Sub

    Private Sub gridlist_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles grdList.KeyDown
        Try
...
#Region "Button Event"
    ...
    Private Sub txt申请者_EditorButtonClick(ByVal sender As Object, ByVal e As Infragistics.Win.UltraWinEditors.EditorButtonEventArgs) Handles txt申请者编号.EditorButtonClick
        ...
        Try
            ...
            If frmSearch.ReturnType = Constant.ENU_MSTSEARCH.SearchMOk Then
                ...
                If Not frmSearch.ReturnTable Is Nothing AndAlso 0 < frmSearch.ReturnTable.Rows.Count Then
                    ...
                    Me.txt订单编号.Text = frmSearch.ReturnTable.Rows(0)("订单编号")     '客户编号
    ...
    Private Sub btn11_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn11.Click
        ...
        Try
            ...
            For index As Integer = 0 To grdList.Rows.Count - 1

                If grdList.Rows(index).Cells("选择").Value = 1 Then
                    objB半成品入库申请T.申请单编号 = grdList.Rows(index).Cells("申请单编号").Value
                    strWhere += ",'" & objB半成品入库申请T.申请单编号 & "'"
                End If
            Next
            If strWhere <> "" Then
                strWhere = "''" + strWhere
            ...
            If dtall.Rows.Count > 0 Then
                Dim rpt As New 半成品入库申请
                Dim rv As New ReportViewr
                rv.ReportExportType = Constant.ReporyType.半成品入库单
...
#Region "ClearForm"
    ...
    Private Sub ClearForm()
        Try
            ...
            Me.txt申请者编号.Text = String.Empty
            Me.txt申请者名称.Text = String.Empty
            Me.dt申请开始日.Text = DateAdd("M", -1, Now)
            Me.dt申请结束日.Text = Date.Now
            Me.txt申请者名称.Text = String.Empty
        ...
        Try
            ...
            If intDoubleClick = 1 Then
                If grdList.ActiveRow.Cells.Count < 10 Then
                    ...
                    Exit Sub
                Else
                    objZ半成品入库申请T.分图号 = Me.grdList.ActiveRow.Cells("分图号").Text.Trim
                End If
            End If
            ...
            '查询の选择されたレCDを取得する
            objZ半成品入库申请T.申请单编号 = Me.grdList.ActiveRow.Cells("申请单编号").Text.Trim

            'テーブルにデータを追加
            ReturnTable = objBll.LoadAllHead(objZ半成品入库申请T)
...
#Region "DataSearch"
    ...
    Private Sub DataSearch()
        ...
        Try
            ...
            objZ半成品入库申请T.申请者编号 = Me.txt申请者编号.Text.Trim
            objZ半成品入库申请T.订单编号 = Me.txt订单编号.Text.Trim
            objZ半成品入库申请T.主图号 = Me.txt产品图号.Text.Trim
            objZ半成品入库申请T.工序名称 = Me.txt工序名称.Text.Trim

            '查询の検索処理

            'mDtSearch = objBll.LoadByWhere(objZ半成品入库申请T)

            If (Not Me.dt申请开始日.Text.Trim = String.Empty) Then
                objZ半成品入库申请T.开始日期 = Me.dt申请开始日.Text.Trim
            Else
                objZ半成品入库申请T.开始日期 = Constant.CGSTRDATE_DEFAULT
            End If

            If (Not Me.dt申请结束日.Text.Trim = String.Empty) Then
                objZ半成品入库申请T.结束日期 = Me.dt申请结束日.Text.Trim
            Else
                objZ半成品入库申请T.结束日期 = Constant.CGSTRDATE_DEFAULT
            End If
            Me.grdList.DataSource = Nothing

            mDt详细 = New DataSet
            Me.mDt详细图纸1 = New DataTable
            mDt详细图纸1 = objBll.LoadByWhere(objZ半成品入库申请T)

            'mdt详细Band1データを記録テーブル初期化
            Me.mDt详细图纸2 = New DataTable
            mDt详细图纸2 = objBll.LoadByWhere1(objZ半成品入库申请T)
