...
#Region "Form Event"
    ...
    Private Sub frm外协品入库申请一览_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub frm外协品入库申请一览_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Try
...
#Region "Button Event"
    ...
    Private Sub txt申请者编号_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txt申请者编号.Validating
...
#Region "InputCheck"
    ...
    Private Function InputCheck() As Boolean
        ...
        Try
            ...
            '必須チェック(MsgID=21) 
            If Not String.IsNullOrEmpty(dmt申请日期end.Text.Trim) And Not String.IsNullOrEmpty(dmt申请日期sta.Text.Trim) Then
                If DateTime.Parse(dmt申请日期sta.DateTime) > DateTime.Parse(dmt申请日期end.DateTime) Then
    ...
    Private Sub Selection()
        ...
        Try
            ...
            If intDoubleClick = 1 Then
                If gridlist.ActiveRow.Cells.Count < 9 Then
                    MsgBox("请选择图号。")
                    Exit Sub
                Else
                    objZ外协品入库申请T.产品图号 = Me.gridlist.ActiveRow.Cells("分图号").Text.Trim
                End If
            End If

            objZ外协品入库申请T.申请单编号 = Me.gridlist.ActiveRow.Cells("申请单编号").Text.Trim


            '查询の选择されたレCDを取得する

            'テーブルにデータを追加
            ReturnTable = objBll.LoadAll(objZ外协品入库申请T)
...
#Region "DataSearch"
    ...
    Private Sub DataSearch()
        ...
        Try
            ...
            objZ外协品入库申请T.申请单编号 = Me.txt申请单编号.Text.Trim
            objZ外协品入库申请T.申请者编号 = Me.txt申请者编号.Text.Trim
            objZ外协品入库申请T.申请日期1 = Me.dmt申请日期sta.Text.Trim
            objZ外协品入库申请T.申请日期2 = Me.dmt申请日期end.Text.Trim
            objZ外协品入库申请T.质检状态 = Me.cmb质检状态.Text

            '查询の検索処理
            'mDtSearch = objBll.LoadByWhere(objZ外协品入库申请T)

            If (Not Me.dmt申请日期sta.Text.Trim = String.Empty) Then
                objZ外协品入库申请T.申请日期1 = Me.dmt申请日期sta.Text.Trim
            Else
                objZ外协品入库申请T.申请日期1 = Constant.CGSTRDATE_DEFAULT
            End If

            If (Not Me.dmt申请日期end.Text.Trim = String.Empty) Then
                objZ外协品入库申请T.申请日期2 = Me.dmt申请日期end.Text.Trim
            Else
                objZ外协品入库申请T.申请日期2 = Constant.CGSTRDATE_DEFAULT
            End If
            Me.gridlist.DataSource = Nothing

            mDt详细 = New DataSet
            Me.mDt详细图纸1 = New DataTable
            mDt详细图纸1 = objBll.LoadByWhere(objZ外协品入库申请T)

            'mdt详细Band1データを記録テーブル初期化
            Me.mDt详细图纸2 = New DataTable
            mDt详细图纸2 = objBll.LoadByWhere1(objZ外协品入库申请T)
