...
#Region "Form Event"
    ...
    Private Sub frm外协品出库申请一览_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub frm外协品出库申请一览_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Try
    ...
    Private Sub frm外协品出库申请一览_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
...
#Region "Selection"
    ...
    Private Sub Selection()
        ...
        Try
            ...
            '查询の选择されたレCDを取得する
            objZ外协品出库申请T.申请单编号 = Me.gridList.ActiveRow.Cells("申请单编号").Text.Trim

            'テーブルにデータを追加
            ReturnTable = objBll.LoadByPKey1(objZ外协品出库申请T)
...
#Region "DataSearch"
    ...
    Private Sub DataSearch()
        ...
        Try
            ...
            objZ外协品出库申请T.申请者编号 = Me.txt申请者编号.Text.Trim
            objZ外协品出库申请T.结束日期 = Me.dt申请开始日期.Text
            objZ外协品出库申请T.结束日期 = Me.dt申请结束日期.Text
            Me.gridList.DataSource = Nothing

            mDt详细 = New DataSet
            Me.mDt详细图纸1 = New DataTable
            mDt详细图纸1 = objBll.LoadByWhere(objZ外协品出库申请T)

            'mdt详细Band1データを記録テーブル初期化
            Me.mDt详细图纸2 = New DataTable
            mDt详细图纸2 = objBll.LoadByWhere1(objZ外协品出库申请T)
    ...
    Private Sub SetGridStyle()
        Try
            ...
            With gridList.DisplayLayout.Bands(1)
                ...
                .Columns("申请数量（个）").CellAppearance.TextHAlign = Infragistics.Win.HAlign.Right
            End With
        Catch ex As Exception
            '異常処理
            ExHelper.ProcessEx(ex)
