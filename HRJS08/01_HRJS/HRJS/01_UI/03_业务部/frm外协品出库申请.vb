...
#Region "Form Event"
    ...
    Private Sub frm外协品出库申请_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub frm外协品出库申请_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
...
#Region "Button Event"
    ...
    Private Sub btnF2编辑_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnF2编辑.Click
        ...
        Dim frmSearch As frm外协品出库申请一览        Try
            frmSearch = New frm外协品出库申请一览            frmSearch.FormOpenMode = Constant.ENU_MODE.Model_Search
    ...
    Private Sub btnF4删除_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnF4删除.Click
        ...
        Dim objZ外协品出库申请T As New Z外协品出库申请T

        Dim objBll As New BLLM外协品出库申请T
        ...
        Try
            ...
            objZ外协品出库申请T.申请单编号 = txt申请单编号.Text.Trim

            If objBll.Delete(objZ外协品出库申请T) > 0 Then
        ...
        Try
            ...
            If frmSearch.ReturnType = Constant.ENU_MSTSEARCH.SearchMOk Then
                ...
                If Not frmSearch.ReturnTable Is Nothing AndAlso 0 < frmSearch.ReturnTable.Rows.Count Then
                    ...
                    '根据检索画面数据进行设置
                    Me.txt申请单编号.Text = frmSearch.ReturnTable.Rows(0)("外协计划单号")     '客户编号
                    SetFormData()
                End If
        ...
        Try
            ...
            If gridList.Rows.Count > 0 Then
                ...
                For i = 0 To gridList.Rows.Count - 1
                    objZ外协品出库申请T = New Z外协品出库申请T
                    objZ外协品出库申请T.申请单编号 = Me.txt申请单编号.Text.Trim
                    objZ外协品出库申请T.申请者编号 = Me.txt人员编号.Text.Trim
                    objZ外协品出库申请T.申请日期 = Me.dt申请日期.Text.Trim
                    objZ外协品出库申请T.备注 = Me.txt备注.Text.Trim
                    objZ外协品出库申请T.订单编号 = gridList.Rows(i).Cells("订单编号").Value.ToString()
                    objZ外协品出库申请T.主图号 = gridList.Rows(i).Cells("主图号").Value.ToString()
                    objZ外协品出库申请T.上级图号 = gridList.Rows(i).Cells("上级图号").Value.ToString()
                    objZ外协品出库申请T.分图号 = gridList.Rows(i).Cells("分图号").Value.ToString()
                    objZ外协品出库申请T.外协品名称 = gridList.Rows(i).Cells("外协品名称").Value.ToString()
                    objZ外协品出库申请T.工序编号 = gridList.Rows(i).Cells("工序编号").Value.ToString()
                    objZ外协品出库申请T.出库数量 = gridList.Rows(i).Cells("出库数量").Value.ToString()
                    objZ外协品出库申请T.外协工序 = gridList.Rows(i).Cells("外协工序").Value.ToString()
                    objZ外协品出库申请T.登陆者 = gObjM_人员T.人员编号
                    objZ外协品出库申请T.登陆日期 = DateTime.Now
                    objZ外协品出库申请T.更新者 = gObjM_人员T.人员编号
                    objZ外协品出库申请T.更新日期 = DateTime.Now
                    arryList.Add(objZ外协品出库申请T)
            ...
            Select Case mIntFormMode
                ...
                Case Constant.ENU_MODE.Model_New
                    ...
                    If objBll.LoadByPKey(objZ外协品出库申请T).申请单编号 <> String.Empty Then
                ...
                Case Constant.ENU_MODE.Model_Update
                    ...
                    If objBll.Delete(objZ外协品出库申请T) > 0 AndAlso objBll.Insert(arryList) > 0 Then
            ...
            Else
                For i = 0 To gridList.Rows.Count - 1
                    ...
                    If gridList.Rows(i).Cells("分图号").Value.ToString() & "" = "" Then
                        ...
                        Return False
                    End If
                    If gridList.Rows(i).Cells("外协品名称").Value.ToString() & "" = "" Then
                        CommonMsg.PFn_ShowMsg(Me.Text, Constant.ENU_MSGID.MSGID13, "外协品名称", "外协品名称")
                        Return False
                    End If
                    ...
                    If gridList.Rows(i).Cells("出库数量").Value.ToString() & "" = "" Then
                        ...
                        Return False
                    End If
                    'If gridList.Rows(i).Cells("客户名称").Value.ToString() & "" = "" Then
                    '    CommonMsg.PFn_ShowMsg(Me.Text, Constant.ENU_MSGID.MSGID13, "客户名称", "客户名称")
                    '    Return False
                    'End If
                    'objZ外协品出库申请T.产品图号 = gridList.Rows(i).Cells("分图号").Value.ToString()
                    'objZ外协品出库申请T.主图号 = gridList.Rows(i).Cells("主图号").Value.ToString()
                    'dt = objBll.Check主图分图(objZ外协品出库申请T)

                    'If dt Is Nothing OrElse dt.Rows.Count = 0 Then
                    '    MsgBox("产品图号：" & objZ外协品出库申请T.产品图号 & "不属于主图号：" & objZ外协品出库申请T.主图号)
                    '    Return False
                    'End If
                Next
            End If
...
#Region "SetFormData"
    ...
    Private Sub SetFormData()
        Dim objZ外协品出库申请T As New Z外协品出库申请T
        Dim objBll As BLLM外协品出库申请T

        Try
            '获取数据
            objZ外协品出库申请T = New Z外协品出库申请T
            objBll = New BLLM外协品出库申请T

            objZ外协品出库申请T.申请单编号 = txt申请单编号.Text.Trim()

            If String.IsNullOrEmpty(objZ外协品出库申请T.申请单编号) Then
                objZ外协品出库申请T.申请单编号 = " "
            End If
            mdsInfoDataSet = objBll.LoadAllHead(objZ外协品出库申请T)
            ...
            End If
            SetGridStyle()
            mdsGridDataSet = objBll.LoadGridAll(objZ外协品出库申请T)
...
#Region "SetData"
    Private Sub SetData()
        ...
        Try
            ...
            objZ外协品出库申请T = New Z外协品出库申请T
            objBll = New BLLM外协品出库申请T
            objZ外协品出库申请T.订单编号 = gridList.ActiveRow.Cells("订单编号").Value.ToString()
            objZ外协品出库申请T.工序编号 = gridList.ActiveRow.Cells("工序编号").Value.ToString()
            objZ外协品出库申请T.分图号 = gridList.ActiveRow.Cells("分图号").Value.ToString()

            If String.IsNullOrEmpty(objZ外协品出库申请T.订单编号) Then
                objZ外协品出库申请T.订单编号 = " "
            End If
            If gridList.Rows(0).Cells("订单编号").Value.ToString <> String.Empty Then

                mDtSearch = objBll.SetData(objZ外协品出库申请T)
...
#Region "Set订单"
    Private Sub Set订单()
        ...
        Try
            ...
            End If
            If gridList.Rows(0).Cells("订单编号").Value.ToString <> String.Empty Then
                mDtSearch = objBll.Set订单(objZ外协品出库申请T)
                If Not mDtSearch Is Nothing And mDtSearch.Rows.Count > 0 Then

                    Me.gridList.ActiveRow.Cells("主图号").Value = mDtSearch.Rows(0)("产品图号")
                    'Me.gridList.ActiveRow.Cells("客户名称").Value = mDtSearch.Rows(0)("客户名称")

                Else
                    Me.gridList.ActiveRow.Cells("主图号").Value = String.Empty
                    ' Me.gridList.ActiveRow.Cells("客户名称").Value = String.Empty
    ...
    Private Sub SetGridStyle()
        Try
            ...
            With gridList.DisplayLayout.Bands(0)
                ...
                .Columns("外协品名称").CellActivation = Activation.NoEdit
                .Columns("出库数量").CellActivation = Activation.NoEdit
                '.Columns("出库数量").CellAppearance.TextHAlign = Infragistics.Win.HAlign.Right
