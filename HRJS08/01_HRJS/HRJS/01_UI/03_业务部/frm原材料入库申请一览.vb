...
#Region "Form Event"
    ...
    Private Sub frm原材料入库申请一览_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        Try
    ...
    Private Sub frm原材料入库申请一览_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        Try
    ...
    Private Sub frm原材料入库申请一览_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        ...
        Try
            ...
            For index As Integer = 0 To gridList.Rows.Count - 1
                ...
                If gridList.Rows(index).Cells("选择").Value = 1 Then
                    objB原材料入库申请T.申请单编号 = gridList.Rows(index).Cells("申请单编号").Value
                    strWhere += ",'" & objB原材料入库申请T.申请单编号 & "'"
                End If
            Next
            If strWhere <> "" Then
                strWhere = "''" + strWhere
            ...
            If dtall.Rows.Count > 0 Then
                Dim rpt As New 原材料入库明细
                Dim rv As New ReportViewr
                rv.ReportExportType = Constant.ReporyType.原材料入库单
    ...
    Private Sub Selection()
        ...
        Try
            ...
            If intDoubleClick = 1 Then
                If Not gridList.ActiveRow.ChildBands Is Nothing Then
                    MsgBox("请选择材料。")
                    Exit Sub
                Else
                    objZ原材料入库申请T.材料编号 = Me.gridList.ActiveRow.Cells("材料编号").Text.Trim
                End If
            End If
...
#Region "DataSearch"
    ...
    Private Sub DataSearch()
        ...
        Try
            ...
            objB原材料入库申请T.申请者编号 = Me.txt申请者编号.Text.Trim()
            objB原材料入库申请T.材料区分 = Me.cob原材料区分.Value
            objB原材料入库申请T.结束日期 = Me.dt申请开始日期.Text
            objB原材料入库申请T.结束日期 = Me.dt申请结束日期.Text
            objB原材料入库申请T.质检状态 = Me.cmb质检状态.Value
            Me.gridList.DataSource = Nothing

            mDt详细 = New DataSet
            Me.mDt详细图纸1 = New DataTable
            mDt详细图纸1 = objBll.LoadByWhere(objB原材料入库申请T)

            'mdt详细Band1データを記録テーブル初期化
            Me.mDt详细图纸2 = New DataTable
            mDt详细图纸2 = objBll.LoadByWhere1(objB原材料入库申请T)
