...
#Region "Button Event"
    ...
    Private Sub btnF11出荷表打印_Click(ByVal sender As Object, ByVal e As EventArgs) Handles F11出荷一览表打印.Click
        ...
        Try
            For Me.i = 0 To gridList.Rows.Count - 1
                If gridList.Rows(i).Cells("选择").Value = True Then
                    PringReport(i)
                End If
            Next
...
#End Region
    Private Sub PringReport(ByRef i As Integer)
        ...
        Dim objModel As B出荷指示T
        objModel = New B出荷指示T
        objModel.订单编号 = gridList.Rows(i).Cells("订单编号").Value.ToString()
        objModel.产品图号 = gridList.Rows(i).Cells("产品图号").Value.ToString()
        objModel.客户名称 = gridList.Rows(i).Cells("客户名称").Value.ToString()
        objModel.订单数量 = gridList.Rows(i).Cells("订单数量").Value.ToString()
        objModel.出荷数量 = gridList.Rows(i).Cells("出荷数量").Value.ToString()
        objModel.出荷预定日 = gridList.Rows(i).Cells("出荷日期").Value.ToString()

        'objModel.产品图号 = gridList.Rows(i).Cells("产品图号").Value
        'objModel.产品图号 = gridList.Rows(i).Cells("产品图号").Value

        rv.ReportExportType = Constant.ReporyType.ProjectRecordT
        rv._ChuHe = objModel
        'rv.ShowDialog()
        rv.Init_Load()
        'rv.Print()
