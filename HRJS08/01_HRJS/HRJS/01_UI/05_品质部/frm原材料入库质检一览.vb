    ...
    Private Sub txt原材料编号_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles txt原材料编号.Leave
        ...
        Try
            ...
            If Me.txt原材料编号.Text.Trim <> String.Empty Then
                ...
                objM原材料T = objBLLM原材料T.LoadByPKey(objM原材料T)

                If objM原材料T.原材料编号 <> String.Empty Then


                    Me.txt原材料名称.Text = objM原材料T.原材料名称      '客户名称
...
#Region "Selection"
    ...
    Private Sub Selection()
        ...
        Try
            ...
            'テーブルにデータを追加
            ReturnTable = objBll.LoadAllHeadzj(objZ原材料入库申请T)
...
#Region "DataSearch"
    ...
    Private Sub DataSearch()
        ...
        Dim objB原材料入库申请T As New B原材料入库质检T
        Dim objBll As New BLLZ原材料入库质检T

        Try
            objB原材料入库申请T = New B原材料入库质检T

            objB原材料入库申请T.材料编号 = Me.txt原材料编号.Text
            objB原材料入库申请T.检查者编号 = Me.txt人员编号.Text
            objB原材料入库申请T.申请单编号 = Me.txt入库申请单.Text.Trim
            objB原材料入库申请T.处理结果 = Me.cmb处理结果.Value
            objB原材料入库申请T.检查日期S = Me.dt检查日期S.Text
            objB原材料入库申请T.检查日期E = Me.dt检查日期E.Text

            Me.gridList.DataSource = Nothing

            Me.mDtSearch = New DataTable
            mDtSearch = objBll.LoadByWhere(objB原材料入库申请T)
